...
export default function ItemCarousel({ itemList = [] }) {
  ...
  const generateBatch = useMemo(() => {
    return () => {
      const allAvailable = itemList.filter(i => !usedIds.includes(getIdFromUrl(i.url)));
  ...
  useEffect(() => {
    ...
    setUsedIds(firstBatch.map(i => getIdFromUrl(i.url)));
  }, [itemList]);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (nearRight || nearLeft) {
        ...
        setUsedIds(prev => {
          const newIds = newBatch.map(i => getIdFromUrl(i.url));
  ...
  return (
    <div className="d-flex align-items-center justify-content-between">
      ...
      <div
        ref={scrollRef}
        className="d-flex overflow-auto px-2 pt-2 pb-2 flex-grow-1 item-carousel"
        style={{ scrollBehavior: 'smooth', gap: '1rem', scrollbarWidth: 'none', msOverflowStyle: 'none' }}
      >
        {batches.flat().map((item, index) => (
          <div key={`${item.name}-${index}`} style={{ flex: '0 0 auto', width: '200px' }}>
